...
export default function HomePage() {
  ...
  const loadArticles = async (page = 1, country?: string) => {
    try {
      ...
      if (response.ok) {
        ...
        // Debug response
        console.log(`Received ${data.articles?.length || 0} articles from API with country: ${country || 'none'}`);
        if (country && data.articles && data.articles.length > 0) {
          console.log(`First article country: ${data.articles[0].country}, id: ${data.articles[0].id}`);
        }
        
        setArticles(data.articles || []);
        setPagination(data.pagination || { page: 1, pageSize: 30, totalCount: 0, totalPages: 0 });
      }
    } catch (error) {
  ...
  const allArticles = [...(articles || []), ...newsApiArticles, ...newsroomArticles];
    
  // Filter articles based on activeFilter
  useEffect(() => {
    // Get all articles from all sources in one unified feed
    const baseArticles = [...(articles || []), ...newsApiArticles, ...newsroomArticles];
    ...
    switch (activeFilter) {
      ...
      case "search":
        ...
        if (searchQuery) {
          filtered = baseArticles.filter(article => 
            ...
            article.aiEnhancedContent?.toLowerCase().includes(searchQuery.toLowerCase())
          );
        } else {
          filtered = baseArticles;
        }
      ...
      case "country":
        ...
        if (selectedCountry) {
          console.log(`Filtering articles client-side for country: ${selectedCountry}`);
          ...
          if (selectedCountry.toUpperCase() === "GLOBAL") {
            ...
            filtered = baseArticles.filter(article => 
              article.country?.toUpperCase() === selectedCountry.toUpperCase()
            );
          }
          
          console.log(`Found ${filtered.length} articles for country ${selectedCountry}`);
        } else {
          filtered = baseArticles;
        }
        break;
    ...
    mutationFn: async ({ 
      ...
      try {
        ...
        if (data && data.articles && Array.isArray(data.articles)) {
          console.log(`Received ${data.articles.length} articles directly from Flask server`);
          ...
          const transformedArticles = data.articles.map((article: any, index: number) => {
            ...
            const transformedArticle = {
              ...
              createdAt: article.publishedAt ? new Date(article.publishedAt).toISOString() : new Date().toISOString(),
              authorId: null
            };
            ...
            try {
              ...
              sessionStorage.setItem(`article-${articleId}`, JSON.stringify(transformedArticle));
            } catch (e) {
        ...
        if (
          ...
          (error?.message && typeof error.message === 'string' && error.message.includes('timeout'))
        ) {
  ...
  useEffect(() => {
    ...
    const backgroundFetchInterval = setInterval(() => {
      ...
      if (!fetchNewsMutation.isPending) {
        ...
        console.log(`Background fetch initiated for ${country.toUpperCase()} news content`);
  ...
  const handleFetchTrendingNews = () => {
    try {
      ...
      const trendingCountries = ["us", "gb", "in", "fr", "jp", "au", "ae", "za"];
  ...
  const handleFetchGlobalNews = () => {
    try {
      ...
      const globalCountries = [
        "gb", "in", "au", "za", "jp", "sg", "fr", "br", 
        "ar", "cn", "eg", "de", "id", "mx", "ru", "ae", 
        "th", "kr", "sa", "ng", "tr", "it", "pl", "ca"
      ];
  ...
  const fetchNewsroomMutation = useMutation({
    mutationFn: async () => {
      try {
        ...
        if (data && data.articles && Array.isArray(data.articles)) {
          console.log(`Received ${data.articles.length} articles from Newsroom API`);
          
          // If no articles were returned, continue silently without notification
          if (data.articles.length === 0) {
            console.log("No articles found from Newsroom API, continuing silently");
          ...
          const transformedArticles = data.articles.map((article: any, index: number) => {
            ...
            try {
              ...
              console.error("Failed to store Newsroom article in sessionStorage:", e);
            }
            
            return transformedArticle;
          });
          
          console.log(`Transformed ${transformedArticles.length} Newsroom articles for display`);
              ...
              <CountryFilter 
                ...
                onChange={(country) => {
                  ...
                  if (country) {
                    ...
                    if (!fetchNewsMutation.isPending) {
                      console.log(`Auto-fetching news for ${country.toUpperCase()}`);
        ...
        {activeFilter !== "all" && (
          <div className="border rounded-md bg-muted/20 p-2 mb-4 flex items-center justify-between">
            <div className="flex items-center gap-2">
              {activeFilter === "hot" && <Flame className="h-4 w-4 text-red-500" />}
              {activeFilter === "global" && <Globe className="h-4 w-4 text-blue-500" />}
              {activeFilter === "search" && <Search className="h-4 w-4 text-purple-500" />}
              {activeFilter === "country" && <MapPin className="h-4 w-4 text-green-500" />}
              
              <span className="text-sm text-muted-foreground">
                {activeFilter === "hot" && "Showing global hot topics and trending news from around the world"}
                {activeFilter === "global" && "Showing global news articles"}
                {activeFilter === "search" && `Search results for: "${searchQuery}"`}
                {activeFilter === "country" && `Showing news from ${selectedCountry?.toUpperCase()}`}
              </span>
            </div>
            <Button 
              variant="ghost" 
        ...
        {(
          ...
          <div className="text-center py-12 border rounded-lg bg-muted/30">
            ...
            <div className="flex gap-2 justify-center">
              ...
              <Button variant="outline" onClick={handleFetchNews}>Fetch More News</Button>
            </div>
          </div>
        )}
      </main>

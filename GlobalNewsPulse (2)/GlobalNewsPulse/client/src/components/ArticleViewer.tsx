...
import { MessageCircle, Send, Wand2, Brain, ListChecks, Share, Radio, Newspaper, Bookmark } from "lucide-react"
...
export function ArticleViewer({ article, onClose }: ArticleViewerProps) {
  ...
  const generateVariants = async () => {
    ...
    try {
      const response = await fetch("/api/variants", {
        ...
        body: JSON.stringify({
          title: article.title,
          content: article.aiEnhancedContent || article.content || article.originalContent
        }),
      });
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          message: userMessage,
          context: article.aiEnhancedContent || article.content || article.originalContent
        }),
      });
  ...
  const handleAIAction = async (action: string) => {
    ...
    switch (action) {
      case "explain":
        prompt = "Please explain the main concepts and key points of this article in simple terms.";
        break;
      case "analyze":
        prompt = "Please provide a detailed analysis of this article, including its implications and potential impact.";
        break;
      case "factcheck":
        prompt = "Please identify and verify the key claims and facts presented in this article.";
        break;
      case "summarize":
        prompt = "Please provide a concise summary of this article highlighting the most important points.";
        break;
    }
        ...
        <Card className="h-full">
          <CardHeader className="pb-2">
            <div className="flex justify-between items-center">
              <div>
                <CardTitle>{decodeHtmlEntities(article.title)}</CardTitle>
          ...
          <CardContent className="pt-2">
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              <TabsList className="w-full grid grid-cols-3">
                ...
                <TabsTrigger value="variants" disabled={!variants}>Content Variants</TabsTrigger>
                <TabsTrigger value="analyze" disabled={!sentiment}>Analysis</TabsTrigger>
              </TabsList>
              ...
              <TabsContent value="read" className="mt-4">
                <ScrollArea className="h-[550px]">
                  <div className="prose prose-sm max-w-none">
                    {article.aiEnhancedContent || article.content || article.originalContent ? (
                      <ReactMarkdown>
                        {decodeHtmlEntities(article.aiEnhancedContent || article.content || article.originalContent || "")}
                      </ReactMarkdown>
                    ) : (
                      <div className="p-4 text-center border border-dashed rounded-md my-8 text-muted-foreground">
                        ...
                        <p className="text-sm">
                          Click the "Generate Variants" button to create enhanced versions of this article using AI.
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <ScrollArea className="h-[550px]">
                  {variants ? (
                    ...
                    <div className="text-center py-20 text-muted-foreground">
                      <p>Click "Generate Variants" to create alternative formats of this article</p>
                    </div>
                  )}
                </ScrollArea>
              </TabsContent>
                  ...
                  {sentiment ? (
                    <div className="space-y-6">
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <Card>
                          <CardHeader>
                            <CardTitle className="text-base">Objectivity Score</CardTitle>
                          </CardHeader>
                          <CardContent>
                            <div className="text-2xl font-bold text-center p-4">
                        ...
                        <CardContent>
                          <div className="flex flex-wrap gap-2">
                            {sentiment.keyThemes.map((theme, i) => (
                              <div key={i} className="px-3 py-1 bg-primary/10 rounded-full text-sm">
                                {theme}
                              </div>
                            ))}
                          </div>
                      ...
                      <Card>
                        <CardHeader>
                          <CardTitle className="text-base">Potential Bias</CardTitle>
                        </CardHeader>
                        <CardContent>
                          <div className="p-4 bg-muted rounded-md">
                    ...
                    <div className="text-center py-20 text-muted-foreground">
                      <p>Click "Analyze" to evaluate sentiment and content themes</p>
                    </div>
                  )}
                </ScrollArea>
              </TabsContent>
        ...
        <Card className="h-full">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-lg">Chat about this article</CardTitle>
              <Menubar>
                <MenubarMenu>
            ...
            <ScrollArea className="flex-1 pr-4 mb-4">
              {messages.length === 0 ? (
                <div className="text-center text-muted-foreground p-4">
                  ...
                  <p className="text-sm">Try the AI Assist menu for preset actions</p>
                </div>
              ) : (
                messages.map((msg, i) => (
                  <Card key={i} className={`mb-4 ${msg.role === 'assistant' ? 'bg-muted' : ''}`}>

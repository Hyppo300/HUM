import { User, Article, Comment, InsertUser, InsertArticle, InsertComment, Login, Register, ForgotPassword, ResetPassword } from "@shared/schema";
...
export interface IStorage {
  ...
  checkVerificationStatus(email: string): Promise<{ isVerified: boolean, canResendAfter?: Date }>;
  resendVerificationEmail(email: string): Promise<boolean>;
  
  // Article methods
  createArticle(article: InsertArticle): Promise<Article>;
  getArticles(options?: { country?: string; page?: number; pageSize?: number }): Promise<{ articles: Article[]; totalCount: number }>;
  getArticleById(id: number): Promise<Article | undefined>;
  getArticleByUrl(url: string): Promise<Article | undefined>;
  updateArticle(id: number, article: Partial<InsertArticle> & { aiEnhancedContent?: string }): Promise<Article>;
...
export class DatabaseStorage implements IStorage {
  ...
  async checkVerificationStatus(email: string): Promise<{ isVerified: boolean, canResendAfter?: Date }> {
    try {
      const user = await this.getUserByEmail(email);
      if (!user) {
      ...
      if (user.verificationTokenExpiry) {
        const tokenSetTime = new Date(user.verificationTokenExpiry.getTime() - 24 * 60 * 60 * 1000); // When token was created
  ...
  async getArticles(options: { 
    ...
    page?: number; 
    pageSize?: number; 
  } = {}): Promise<{ articles: Article[]; totalCount: number; totalPages?: number; page?: number; pageSize?: number }> {
    try {
      ...
      const quickCountPromise = Promise.race([
        ...
        new Promise((_, reject) => setTimeout(() => reject(new Error("Quick count timeout")), 2000))
      ]);
      ...
      try {
        ...
        console.log(`Quick count found ${quickCount} articles, proceeding with full query`);
      } catch (err) {
      ...
      try {
        const articlesPromise = Promise.race([
          articlesQuery.orderBy(desc(articles.createdAt)).limit(pageSize).offset(offset),
          new Promise((_, reject) => setTimeout(() => reject(new Error("Articles query timeout")), 5000))
        ]);
        ...
        console.error(`Articles query timed out, returning empty array: ${queryErr}`);
        return { 
          articles: [], 
          totalCount: 0,
          totalPages: 0,
...
export class MemStorage implements IStorage {
  ...
  private initializePlaceholderData() {
    ...
    // Global news
    this.articles.set(1, createPlaceholderArticle(
      1,
      "Global Climate Summit Achieves Breakthrough Agreement",
      "World leaders have reached a historic climate agreement at the Global Climate Summit, pledging to reduce carbon emissions by 50% by 2030.",
      "GLOBAL",
      "environment"
    ));

    this.articles.set(2, createPlaceholderArticle(
      2,
      "Tech Giants Announce Joint Initiative for Quantum Computing",
      "Major technology companies have formed an alliance to accelerate quantum computing research and development.",
      "GLOBAL",
      "technology"
    ));

    // Global trending news
    this.articles.set(3, createPlaceholderArticle(
      3,
      "Breaking: Artificial Intelligence Makes Major Medical Discovery",
      "An AI system has identified a new antibiotic compound that can combat drug-resistant bacteria, marking a significant advancement in medical research.",
      "GLOBAL",
      "science",
      true
    ));
    ...
    // US news
    this.articles.set(4, createPlaceholderArticle(
      4,
      "US Economy Shows Strong Growth in Second Quarter",
      "The United States economy exceeded expectations with a 4.2% growth rate in the second quarter, marking the strongest performance in two years.",
      "US",
      "business"
    ));

    // US trending news
    this.articles.set(5, createPlaceholderArticle(
      5,
      "Historic Infrastructure Bill Passes in US Congress",
      "After months of negotiation, the US Congress has passed a $1.2 trillion infrastructure bill that will fund critical projects across the country.",
      "US",
      "politics",
      true
    ));
    ...
    // UK news
    this.articles.set(6, createPlaceholderArticle(
      6,
      "UK Announces New Green Energy Initiative",
      "The United Kingdom has unveiled an ambitious plan to power all homes with offshore wind energy by 2030, creating thousands of new jobs.",
      "GB",
      "environment"
    ));

    // Japan news
    this.articles.set(7, createPlaceholderArticle(
      7,
      "Japan's Central Bank Adjusts Monetary Policy",
      "The Bank of Japan has made a significant shift in its monetary policy, allowing government bond yields to move in a wider range.",
      "JP",
      "business"
    ));

    // India trending news
    this.articles.set(8, createPlaceholderArticle(
      8,
      "India Launches Record-Breaking Satellite Constellation",
      "India's space agency has successfully launched 104 satellites in a single mission, setting a new world record for satellite deployment.",
      "IN",
      "science",
      true
    ));
    ...
    // Germany news
    this.articles.set(9, createPlaceholderArticle(
      9,
      "German Auto Industry Accelerates Electric Vehicle Production",
      "Major German automakers have announced plans to significantly increase electric vehicle production in response to growing demand and environmental regulations.",
      "DE",
      "business"
    ));

    // Brazil news
    this.articles.set(10, createPlaceholderArticle(
      10,
      "Brazil Unveils New Conservation Plan for Amazon Rainforest",
      "The Brazilian government has introduced a comprehensive plan to protect the Amazon rainforest, including stricter enforcement against illegal logging.",
      "BR",
      "environment"
    ));
  }
  ...
  async resendVerificationEmail(email: string): Promise<boolean> {
    ...
    // In memory storage, we'll just log instead of sending email
    console.log(`Verification email resent for ${email}, token: ${verificationToken}`);
  ...
  async createUser(insertUser: InsertUser): Promise<User> {
    ...
    console.log(`Memory storage: User created with email ${insertUser.email}, verification token: ${verificationToken}`);
    return user;
  }
  ...
  async getArticles(options?: { country?: string; page?: number; pageSize?: number }): Promise<{ articles: Article[]; totalCount: number }> {
    const { country, page = 1, pageSize = 30 } = options || {};
    const offset = (page - 1) * pageSize;
    ...
    if (country) {
      ...
      if (country.toUpperCase() === "GLOBAL") {
        ...
        filteredArticles = filteredArticles.filter(article => 
          article.country?.toUpperCase() === country.toUpperCase()
        );
      }
    }
  ...
  async updateArticle(id: number, article: Partial<InsertArticle> & { aiEnhancedContent?: string }): Promise<Article> {
